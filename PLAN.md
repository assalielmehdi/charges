...
In:
1. Login page (Supabase Auth, email/password for a single allowlisted email) + middleware that gates every route.
...
| Audience | Single user. Supabase Auth (email/password), single allowlisted email enforced server-side. `middleware.ts` redirects unauthenticated requests to `/login`. |
...
2. ~~**Auth + schema.** Postgres schema (`expenses`, `categories`), seed categories, RLS enabled, `/login` email/password page (Supabase Auth via `@supabase/ssr`), `middleware.ts` refreshes session and gates every route, single-email allowlist enforced server-side.~~ ✅ Migrations applied (`0001_init.sql`, `0002_auth_rls.sql`), Supabase Auth configured (signups disabled, single user only, email/password login), env vars set in `.env.local` and Vercel. End-to-end login verified locally.
...
6. ~~**Scan flow.** `/api/scan` route → upload to Supabase Storage → Claude vision call (multi-lingual prompt) → review-before-save form pre-filled with extracted fields → save.~~ ✅ `/expenses/scan` capture page POSTs to `/api/scan` (auth-gated by `requireAuthorizedUser`, allowlist enforced server-side); route uploads bytes to private `receipts/{user_id}/{uuid}.{ext}` bucket and calls Claude Haiku 4.5 vision with a tool-use schema `{amount, date, merchant, category_hint}` (AR/FR/EN, MAD assumed, null for low-confidence). Extraction is stashed in `sessionStorage` and the user lands on `/expenses/scan/review?id=…`, where a signed-URL photo preview sits above a pre-filled `ExpenseFormFields` (category_hint mapped to closest existing category, fallback to last-used). Save inserts with `source='scan'`, `photo_path`, `extraction_raw=<full Claude response>`. Cancel deletes the storage object (delete-on-cancel); tab-close orphans accepted for MVP. Storage bucket + RLS provisioned by `0003_storage.sql`. Edit page (`/expenses/[id]`) renders the receipt photo above the form via a 10-minute signed URL when `photo_path` is set.
